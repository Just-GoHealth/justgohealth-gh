    ...
    <div ref={containerRef} className="relative flex flex-col overflow-y-auto h-screen">
      <div className="px-6 pt-16">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, transition: { duration: 1.5 } }} className="flex items-center gap-3">
          <h2 className="text-3xl text-black font-bold">meet</h2>
          <div className="bg-blue-900 px-3 py-1">
            <h3 className="text-3xl italic text-white font-bold">Dr. Nathaniel Codjoe</h3>
          </div>
        </motion.div>
      </div>
      ...
      <div className="relative w-full mt-4">
        <div className="relative w-full aspect-[4/5]">
          <Image src="/hero/doc-boy.png" alt="Dr. Nathaniel Codjoe" fill className="object-contain" priority />
        </div>
      ...
      <div className="px-6 pb-12">
        <motion.p className="text-[20px] text-black font-bold mt-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0, transition: { delay: 3.5, duration: 1.2 } }}>
          What does it mean to lock in?
        </motion.p>
        <motion.div className="mt-4 space-y-3">
          {["Attended St. Peter's Boys'", "Got into Medical School", "Graduated top of his class", "Won 16 out of 22 awards"].map((text, i) => (
            <motion.div
              key={text}
        ...
        <motion.div 
          className="bg-blue-900 px-6 py-3 rounded-full w-fit mt-6 mx-auto" 
