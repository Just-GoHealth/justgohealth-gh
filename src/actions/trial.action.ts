...
export const getSchools = async function () {
  const res = await fetch("https://lockin-backend-production.up.railway.app/api/v1/schools");
...
export const sendTrialRequest = async (data: ITrial) => {
  const res = await fetch("https://lockin-backend-production.up.railway.app/api/v1/lockin", {
    method: "POST",
    headers: {
